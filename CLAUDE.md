...
- **Próximo:** Fase 2c (admin CRUD custom de las 7 colecciones + Server Action de contacto que persiste + email Resend tras env var). Plan: `docs/superpowers/plans/2026-06-30-stage-2-site-and-admin.md`.
  - **Pendiente del cliente para 2c (decisión):** subir el límite de archivo de PocketBase de 5MB → 15MB (`pocketbase/set-file-limits.mjs`, requiere su OK explícito; el seed usó placeholders ya optimizados <2MB para no toparlo).
